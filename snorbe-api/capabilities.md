...
| 「レポート作成」「報告書」「まとめて」／数字番号セクション／Part A/B/C 構造／長文構造化 inputText | `report`（HITL 構造確認） ⚠️ 誤発火注意 |
...
### ⚠️ `report` は opt-in で扱う (呼び出し側の責任)

`report` は **HITL 構造確認 + 各セクション執筆** で通常の 5〜10 倍のコスト・時間がかかる。ユーザーが明示的に「レポートで欲しい」と言った時**だけ**発火させる。

- **デフォルトは調査回答 (会話形式)**、レポートではない
- **呼び出し側 (エージェント) はユーザーに「調査だけ？レポートまで？」を事前ヒアリング**する
- 長文構造化 inputText で意図せず発火する事故が多い — 対策は [prompting.md#長文構造化-inputtext-で-report-を意図せず発火させる](prompting.md) 参照
- 「レポート不要」と inputText に書いても構造で誤ルートするので、**構造そのものを短く保つ**のが唯一確実な回避策
...
`modelName` は **必須**。以下から選ぶ。**API 経由では原則 `snorbe-fast`（高速・低コスト）、`snorbe-medium`（バランス・リーズナブル）、または `snorbe-quality`（高精度・詳細）を指定する。** これらはプロンプトごとに Snorbe 内部で最適プロバイダモデルへ自動マッピングされる。
