...
### ビジュアルマップ生成（`visualmap` HITL）

```
/visualmap
ワークスペース内の生成 AI 関連エンティティを
「成熟度 × 適用領域」の 2 軸マップにして、 メーカーで色分けしてください。
```

誘導要素:
- `/visualmap` スラッシュ、 もしくは「軸マップ」「散布図」「2軸/3軸で配置」「色分け」 → `visualmap` 発火
- 軸候補（X / Y / 必要なら Z / W）と色・形・サイズ・ラベルに使う属性を **メタデータキー名**で示すと stage-1 の質問が減る
- 対象エンティティは ID を書かなくてよい — 名前/ラベル/カテゴリ/期間で範囲を示せば、 server が DB 上の CUID に解決する（drag-and-drop も含めて以後はすべて CUID 基準で動く）
- HITL: 軸構成のドラフト → 質問に回答 or confirm → 全エンティティ × 全軸のメタデータを stage-2 LLM で抽出し、 GraphView として保存される
...
plan / report / matrix / visual-map を使わせたい場合、**ユーザーが確認ステップに応答する前提**で書く:
