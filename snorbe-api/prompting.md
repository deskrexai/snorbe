...
### 調査のみで済ませたい場合（`report` を発火させない書き方）

**デフォルトはこちら**。ほとんどの調査依頼は、レポート HITL を経由せず、単一の会話回答で済む。

```
2024年時点の自動運転業界の主要プレイヤーと規制動向をざっと調べて、
根拠URL付きで日本語で回答してください。長文レポートは不要、
要点をリスト形式でお願いします。
```

**`report` を回避するコツ**:

- **短く、単一のクエスチョンとして書く** (300字以内推奨、複雑でも1000字以内)
- **「回答してください」「教えてください」で締める** (「まとめてください」は避ける — `report` に誤ルートする)
- **「レポート不要」「短い形式で」「要点だけ」「会話形式で」を明示** すると効く
- **セクション番号 (1. 2. 3.) や Part A / Part B の構造を避ける** (この構造は `report` トリガーになる)
- **「成果物として」「章立てで」「セクション構成で」を書かない**

**うっかり `report` が発火する典型**:

```
❌ Anima LoRA について以下を調査してほしい:
   1. データセット要件
   2. 学習ツール比較
   3. 推奨パラメータ
   4. 失敗診断
...
長い構造化 inputText は「レポート不要」と明記しても Snorbe の chat-routing が `report` に誤ルートすることがある。 **一発質問** に分解し、深掘りが必要なら追加でもう一度投げる方が安全かつ早い。

```
✅ Anima LoRA 学習の「推奨パラメータ (rank/LR/steps/optimizer)」だけ、
   実測ソース (note.com / Civitai / Reddit) から根拠URL付きで教えてください。
   会話回答で結構、レポート不要。
```

### レポート生成（`report` HITL） — ★ ユーザーが明示的に希望した時のみ

**必ずユーザーへの事前ヒアリングを済ませてから使う** ([SKILL.md#最初にユーザーへ確認-調査だけレポートまで](SKILL.md) 参照)。ユーザーが「レポートで欲しい」と言った時、あるいは「後日再利用する文書として残したい」と明言した時**だけ** `report` を意図的に発火させる。
...
**コストの目安** (`snorbe-quality` model, 4 sections):
- 単一調査回答: 1 SSE ラウンド、1〜3 分、~$0.5-2
- フルレポート: 3〜4 ラウンド (plan-HITL + report-structure-HITL + section 執筆), 10〜30 分, ~$3-15
- **レポートは調査の 5〜10 倍のコスト**。ユーザーが本当に欲しい時だけ
...
### ❌ 長文構造化 inputText で `report` を意図せず発火させる

Snorbe の `chat-routing` は **inputText の構造・長さで `report` に誤ルート**しやすい。以下のような書き方は、たとえ「レポート不要」と明記していても `report-structure-draft` HITL に入ってしまう:
...
❌ 以下を調査してほしい:
   ...
   ...成果物として、以下の形式でまとめてほしい:
   - チェックリスト
   - パラメータ表
   - コマンド例

   フルレポート形式ではなく凝縮形式で。   ← ★「レポート不要」と書いても発火する
```

**発火するトリガー要因**:
- 数字番号付きセクション (`1. XXX / 2. YYY`) や Part A/B/C 構造
- 「以下を調査」「以下の観点で」「以下を含めて」の羅列
- 「成果物として」「章立てで」「セクション構成」の語彙
- 全体で 1500 字を超える長文 inputText

**対処**:
1. **1つの調査は1つの短いクエスチョンに分解する** (「XXX について要点だけ」レベル)
2. **フォローアップは会話ラウンドを重ねる** (Snorbe は同じ workspace 内でナレッジグラフに蓄積するので、追加質問で深掘れる)
3. **どうしても長文構造で投げたい場合**: 事前にユーザーに「これはレポートモードで走らせて良い？」と確認
