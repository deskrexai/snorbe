...
今回のプロジェクトで動いている特許請求項抽出用の prompt（[recipes/patent-batch-extraction.md](recipes/patent-batch-extraction.md) 参照）:

````
Google Patents で {公開番号} の全請求項を取得し、以下の JSON 形式のみで出力せよ:
...
**機能する理由**:
- 「Google Patents で」「取得」→ `search` または `browse` を誘導
- JSON スキーマを明示 → 構造化パース可能
- 「他のテキスト不要」相当の制約 → 前置きが減る
- 翻訳＋分類を1回で要求 → 追加 run 不要
...
3. **`GET /chat/list` で `agentRun.process` を見る** → どのツールが実際に呼ばれたか確認
