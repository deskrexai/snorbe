...
切断されても **サーバー側は最後まで走り切る**（結果は `/chat/list` に残る）が、
...
## ラン完了後の詳細取得（`/chat/list`）

`/chat/list` は**ラン後の詳細データを取得する主要エンドポイント**。
`chats[].agentRun` に以下が丸ごと含まれる:
...
# 直近のランを取得
resp = requests.get(f"{BASE}/chat/list?limit=20", headers=HEADERS)
data = resp.json()
...
特定 `runId` を狙い撃ちしたい場合は `/chat/list` を **降順でページング** して
`chats[].agentRun.id === 目的のrunId` で探す（runId 直接取得のエンドポイントは未提供）。
...
これは `process` を含まないので軽量。本文や詳細が必要なら `/chat/list` を使う。
